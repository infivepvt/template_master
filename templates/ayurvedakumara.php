...
<body>
  ...
  <div class="name-section">
    <h4 style="color:#5b0e2e;font-weight :bold">Mr.Priyantha Kumara  </h4>
  ...
  <div class="container my-4">
    <div class="row text-center contact-box mx-1 justify-content-center">
      <div class="col-4 mb-3" onclick="window.open('tel:0704725633 ', '_self')">
      ...
      <div class="col-4 mb-3" onclick="window.open('https://wa.me/94704725633 ', '_blank')">
        <i class="fab fa-whatsapp contact-icon"></i><br />
        <span>Whatsapp</span>
      </div>
      <div class="col-4 mb-3" onclick="window.open('mailto:kumara@adhitya.lk', '_self')">
...
<script>
  async function generateVCF() {
    // Updated contact information to match the HTML content
    const contactData = {
      firstName: "Priyantha",
      lastName: "Kumara",
      title: "Finance Manager",
      phoneWork: "0252030200",
      phoneMobile: "0704725633",
      email: "kumara@adhitya.lk",
    ...
    try {
      ...
      await Promise.all([
        new Promise((resolve) => {
          if (profileImage.complete) resolve();
          else {
        ...
        new Promise((resolve) => {
          if (logoImage.complete) resolve();
            ...
            logoImage.onerror = resolve; // Handle error case
          }
        })
      ]);

      // Create canvas and draw profile image
      const profileCanvas = document.createElement('canvas');
      const profileCtx = profileCanvas.getContext('2d');
      profileCanvas.width = profileImage.naturalWidth;
      profileCanvas.height = profileImage.naturalHeight;
      profileCtx.drawImage(profileImage, 0, 0);
      const profileBase64 = profileCanvas.toDataURL('image/jpeg').split(',')[1];

      // Create canvas and draw logo image
      ...
      const logoBase64 = logoCanvas.toDataURL('image/jpeg').split(',')[1];

      // Create VCF content with base64 images
...
PHOTO;ENCODING=b;TYPE=JPEG:${profileBase64}
LOGO;ENCODING=b;TYPE=JPEG:${logoBase64}
END:VCARD`;
      ...
      // Fallback to simple VCF without images if there's an error
